  ...
  <label for=case>Obudowa</label><select id=case><option value>Wybierz obudowe</option>''' + case_options + '''</select>
     ...
     <output id=budget-result>Podaj budzet jako nieujemna calkowita kwote w PLN.</output>
      ...
      <script>document.addEventListener('click',async event=>{if(event.target!==openConfiguration)return;event.stopImmediatePropagation();const configurationIdValue=configurationIdInput.value.trim();if(!configurationIdValue){result.textContent='Podaj identyfikator konfiguracji.';result.dataset.level='blocking';return}const response=await fetch('/api/configurations/'+encodeURIComponent(configurationIdValue));const configuration=await response.json();if(response.ok===false){result.textContent=configuration.error||'Nie udalo sie otworzyc konfiguracji.';result.dataset.level='blocking';return}applyConfiguration(configuration);refresh()},{capture:true})</script>
       ...
       <script>const savedConfigurations=document.querySelector('#saved-configurations');const loadSavedConfigurations=async()=>{try{const response=await fetch('/api/configurations');const configurations=await response.json();savedConfigurations.textContent='';if(!configurations.length){const empty=document.createElement('li');empty.textContent='Brak nazwanych zapisow do wyboru.';savedConfigurations.append(empty);return}configurations.forEach(configuration=>{const entry=document.createElement('li');entry.dataset.configurationId=configuration.configuration_id;entry.textContent=configuration.name;savedConfigurations.append(entry)})}catch(error){savedConfigurations.textContent='';const failure=document.createElement('li');failure.textContent='Nie udalo sie wczytac zapisanych wariantow.';savedConfigurations.append(failure)}};savedConfigurations.addEventListener('click',async event=>{const entry=event.target.closest('[data-configuration-id]');if(!entry)return;const response=await fetch('/api/configurations/'+encodeURIComponent(entry.dataset.configurationId));const configuration=await response.json();if(response.ok===false){result.textContent=configuration.error||'Nie udalo sie otworzyc konfiguracji.';result.dataset.level='blocking';return}applyConfiguration(configuration);await refresh()});loadSavedConfigurations()</script>
           </main></body></html>'''
